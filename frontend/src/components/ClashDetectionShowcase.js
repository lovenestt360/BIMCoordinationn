...
const ClashStaticFallback = () => (
  ...
  >
    <div
      className="absolute inset-0"
      style={{ background: 'linear-gradient(180deg, rgba(2,6,23,0.55) 0%, rgba(2,6,23,0.88) 100%)' }}
    />
    <div className="absolute inset-0 flex items-center justify-center gap-3 px-4">
      {STATUS_STEPS.map((s) => (
        <div
          key={s.label}
          className="output-tag-glow px-4 py-2 rounded-full text-xs font-mono font-bold"
  ...
  return (
    ...
    >
      ...
      <div className="sr-hidden relative z-10 lg:col-span-2 flex flex-col">
        ...
        <p className="text-[#94A3B8] text-sm leading-relaxed flex-1 mb-5">{step.description}</p>
      ...
      <div
        ...
        style={{ borderColor: 'rgba(14,165,233,0.15)', background: '#050B16' }}
      >
        <CanvasWrapper
          className="absolute inset-0"
